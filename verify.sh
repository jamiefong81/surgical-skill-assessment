...
#
# Prerequisites:
#   - LOSO fold checkpoints in models/ (run: python3 train.py --regression)
...
# so repeated runs with different settings never overwrite each other.
# The baseline (T=10, exact, n2v) always writes to regression_results.txt.
...
echo "=== Settings: T=${T}  method=${METHOD}  verifier=${VERIFIER} ==="
echo ""
...
if [ "$missing" -ne 0 ]; then
    echo "ERROR: train first:  python3 train.py --regression"
    exit 1
fi
...
echo "=== Step 1: Export each fold to ONNX (T=${T}) ==="
for i in $FOLDS; do
